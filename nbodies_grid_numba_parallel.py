...
# On utilise numba pour accélérer les calculs (version parallèle : parallel=True + prange).
import numpy as np
import visualizer3d
import sys
import time
...
# Modification par rapport à la version séquentielle :
# cette fonction est compilée avec Numba en mode parallèle.
@njit(parallel=True)
def update_stars_in_grid( cell_start_indices : np.ndarray, body_indices : np.ndarray,
    ...
    # (boucle séquentielle : écriture concurrente sur cell_counts impossible à paralléliser)
    ...
    # Calcule les indices de début des cellules (préfixe sum, séquentiel)
    running_index = 0
    ...
    # Remplit les indices des corps dans les cellules (séquentiel : dépendance sur current_counts)
    ...
    # Partie parallélisée :
    # chaque cellule est traitée indépendamment pour calculer sa masse totale et son centre de masse.
    # La boucle range de la version séquentielle est remplacée ici par prange.
...
# Modification par rapport à la version séquentielle :
# cette fonction utilise aussi parallel=True pour paralléliser le calcul de l'accélération.
@njit(parallel=True)
def compute_acceleration( positions : np.ndarray, masses : np.ndarray,
    ...
    # Partie parallélisée : chaque corps calcule son accélération indépendamment
...
class NBodySystem:
    ...
    # Accumulateurs de temps pour mesure de performance
    ...
    def update_positions(self, dt):
        ...
        NBodySystem.total_compute_time += (t1 - t0)
        NBodySystem.n_steps += 1
        if NBodySystem.n_steps % 20 == 0:
            avg = NBodySystem.total_compute_time / NBodySystem.n_steps
            print(f"[step {NBodySystem.n_steps}] avg compute time per step: {avg*1000:.2f} ms")
